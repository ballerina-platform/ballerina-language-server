...
/**
 * Builder for the WhatsApp Business event-integration service. Generates a {@code whatsapp:Listener}
 * plus an empty {@code whatsapp:WhatsAppService} on it; the individual webhook event handlers
 * (onMessages, onMessageTemplateStatusUpdate, ...) are added on demand through the "Add Handler"
 * panel, mirroring the RabbitMQ event-integration UX. The {@code ballerinax/whatsapp.business}
 * module is imported with the {@code whatsapp} alias (its natural prefix would be {@code business}),
 * matching the module's own documented usage.
 *
 * <p>The {@code ballerinax/whatsapp.business} module is published to Ballerina Central (version
 * 2.0.0). The trigger metadata (service declaration, handler signatures, init form) is
 * hand-authored in {@code service_artifacts.json} rather than derived purely from the semantic
 * model, since {@code addServiceInitSource} needs protocol-specific source generation (aliased
 * import, named listener args) regardless of publish status.
 *
 * @since 1.8.0
 */
public class WhatsAppBusinessServiceBuilder extends AbstractServiceBuilder {
    ...
    private static final String MODULE_NAME = "whatsapp.business";
    private static final String ALIAS = "whatsapp";
    private static final String LISTENER_TYPE = "whatsapp:Listener";
    private static final String SERVICE_TYPE = "whatsapp:WhatsAppService";
    ...
    public Map<String, List<TextEdit>> addServiceInitSource(AddServiceInitModelContext context)
        ...
        if (!importExists(modulePartNode, ORG, MODULE_NAME)) {
            edits.add(new TextEdit(Utils.toRange(modulePartNode.lineRange().startLine()),
                    String.format(ALIASED_IMPORT_TEMPLATE, ORG, MODULE_NAME, ALIAS)));
        }
