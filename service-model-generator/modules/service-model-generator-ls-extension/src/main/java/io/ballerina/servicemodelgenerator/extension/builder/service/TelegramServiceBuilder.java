...
/**
 * Builder for the Telegram event-integration service. Generates a {@code telegram:Listener} plus an
 * empty {@code telegram:TelegramService} on it; the individual bot event handlers (onMessage,
 * onCallbackQuery, ...) are added on demand through the "Add Handler" panel, mirroring the RabbitMQ
 * event-integration UX.
 *
 * <p>The {@code ballerinax/telegram} module is published to Ballerina Central (version 0.9.0). The
 * trigger metadata (service declaration, handler signatures, init form) is hand-authored in
 * {@code service_artifacts.json} rather than derived purely from the semantic model, since
 * {@code addServiceInitSource} needs protocol-specific source generation (flattening
 * {@code ListenerConfig} fields into named args, omitting blanks) regardless of publish status.
 *
 * @since 1.8.0
 */
public class TelegramServiceBuilder extends AbstractServiceBuilder {
    ...
    private static final String MODULE_NAME = "telegram";
